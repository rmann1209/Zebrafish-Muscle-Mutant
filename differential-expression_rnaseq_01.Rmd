...
data_dir <- file.path("data", "ERP009868")

# Declare the file path to the gene expression matrix file
# inside directory saved as `data_dir`
# Replace with the path to your dataset file
data_file <- file.path(data_dir, "ERP009868.tsv")

# Declare the file path to the metadata file
# inside the directory saved as `data_dir`
# Replace with the path to your metadata file
metadata_file <- file.path(data_dir, "metadata_ERP009868.tsv")
```
...
# Mapping ensembl codes to gene names


```{r}
# Install the AnnotationDbi package
...
```{r}
data_file.df <- as.data.frame(data_file)
```
...
```{r}
metadata <- metadata %>%
  # Let's get the mutation status from this variable
  dplyr::mutate(mutation_status = dplyr::case_when(
    stringr::str_detect(refinebio_title, "ZMP") ~ "mutation",
    stringr::str_detect(refinebio_title, "Danio") ~ "reference"
  ))
```
...
metadata <- metadata %>%
  dplyr::mutate(
    ...
    mutation_status = factor(mutation_status, levels = c("reference", "mutation"))
  )
```
...
filtered_mapped_df <- final_mapped_df %>%
  dplyr::filter(rowSums(final_mapped_df[4:1889]) >= 10)
```
...
gene_matrix <- round(final_mapped_df[4:1889])
```
